        ...
        {question.type === QuestionType.RATING_MATRIX && (
          <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
            {question.subQuestions?.map((sub: SubQuestion) => {
               const currentVal = (value as {[key: string]: number})?.[sub.id]; // No default, must select
               return (
                 <div key={sub.id} className="bg-white/5 p-5 rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                   ...
                   <div className="flex items-center justify-between mb-2 px-1">
                     <span className="text-[10px] text-gray-500 font-mono uppercase tracking-wider">Lowest (1)</span>
                     <span className="text-[10px] text-gray-500 font-mono uppercase tracking-wider">Best (5)</span>
                   </div>
