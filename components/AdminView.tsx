...
import { ArrowLeft, Lock, Trash2, Database, AlertCircle, FileJson, ChevronDown, ChevronUp } from 'lucide-react';
...
// Mock Data generation for visualization testing
const generateMockData = (count: number = 20): SurveyData[] => {
  const data: SurveyData[] = [];
  for(let i=0; i<count; i++) {
    data.push({
      id: `mock_${Date.now()}_${i}`,
      timestamp: Date.now(),
      path: Math.random() > 0.3 ? 'middle_school' : 'university',
      answers: {
        'q0_grade': Math.random() > 0.7 ? 'uni' : 'high',
        'q2_freq': ['daily', 'weekly', 'rarely'][Math.floor(Math.random() * 3)],
        'q4_essay': ['proxy', 'collage', 'mentor', 'trad'][Math.floor(Math.random() * 4)]
      }
    });
  }
  return data;
...
const AdminView: React.FC<AdminViewProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    // Load real local storage data only
  ...
  const handleClearDatabase = () => {
    if (window.confirm('WARNING: This will permanently delete ALL survey records. This action cannot be undone. Continue?')) {
  ...
  const handleAddMockData = () => {
    ...
    setData(updated);
    // Optional: Don't save mock data to localStorage to keep DB clean on refresh, 
    // or uncomment next line to persist it.
    // localStorage.setItem('survey_responses', JSON.stringify(updated));
  };
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-md bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-2xl relative overflow-hidden group">
          ...
          <button onClick={onBack} className="w-full mt-6 text-slate-500 hover:text-cyan-400 transition-colors text-xs font-mono flex items-center justify-center gap-2">
            <ArrowLeft size={12} /> RETURN_TO_PUBLIC
          </button>
        </div>
      </div>
    );
  ...
  const pathStats = [
    { name: 'Middle/High School', value: data.filter(d => d.path === 'middle_school').length },
  ...
  const styleStats = [
    ...
    { name: 'Traditional', value: data.filter(d => d.answers['q4_essay'] === 'trad').length },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 p-6 font-mono">
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 max-w-6xl mx-auto gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">SECURE_ADMIN_PANEL_V1.0</p>
        </div>
      ...
      {data.length === 0 ? (
        <div className="max-w-6xl mx-auto min-h-[400px] flex flex-col items-center justify-center border border-slate-800 border-dashed rounded-xl bg-slate-900/30">
          ...
          <h3 className="text-xl text-slate-400 mb-2">DATABASE_EMPTY</h3>
          <p className="text-slate-600 text-sm">No survey responses found. Waiting for input...</p>
        </div>
      ) : (
        <>
          <main className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 relative overflow-hidden group hover:border-cyan-500/30 transition-colors">
              ...
              <h3 className="text-slate-400 text-xs tracking-widest mb-2">TOTAL_RESPONSES</h3>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-cyan-500/30 transition-colors">
              <h3 className="text-slate-400 text-xs tracking-widest mb-4">DEMOGRAPHIC_SPLIT</h3>
              <div className="h-64">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-cyan-500/30 transition-colors">
              <h3 className="text-slate-400 text-xs tracking-widest mb-4">AI_USAGE_FREQUENCY</h3>
              <div className="h-64">
                 <ResponsiveContainer width="100%" height="100%">
            ...
            {/* Card 4: Essay Style */}
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-cyan-500/30 transition-colors">
              <h3 className="text-slate-400 text-xs tracking-widest mb-4">WRITING_HABITS_DISTRIBUTION</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
          ...
          <section className="max-w-6xl mx-auto mt-8 border-t border-slate-800 pt-8 pb-12">
            <h2 className="text-xl font-bold text-cyan-400 mb-6 flex items-center gap-2">
              <FileJson size={20} /> DETAILED_DATA_LOG
            </h2>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 mt-2 md:mt-0">
                      <span className={`text-xs px-2 py-1 rounded border ${
                        ...
                        {record.path === 'university' ? 'UNIVERSITY' : 'MIDDLE_SCHOOL'}
                      </span>
