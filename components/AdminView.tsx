...
const AdminView: React.FC<AdminViewProps> = ({ onBack }) => {
  ...
  const pathStats = [
    { name: 'Middle', value: data.filter(d => d.path === 'middle_school').length },
    { name: 'High School', value: data.filter(d => d.path === 'high_school').length },
    { name: 'University', value: data.filter(d => d.path === 'university').length },
    { name: 'Teacher', value: data.filter(d => d.path === 'teacher').length },
  ];
        ...
        <>
          <main className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-cyan-500/30 transition-colors">
              ...
              <div className="flex justify-center gap-4 text-[10px] text-slate-400 mt-2 uppercase flex-wrap">
                {pathStats.map((entry, index) => (
                    ...
                    <div className="flex items-center gap-4 mt-2 md:mt-0">
                      <span className={`text-xs px-2 py-1 rounded border ${
                        record.path === 'university' 
                          ...
                          : record.path === 'high_school' 
                            ? 'border-green-500/30 text-green-400 bg-green-500/10'
                            : record.path === 'teacher'
                              ? 'border-pink-500/30 text-pink-400 bg-pink-500/10'
                              : 'border-cyan-500/30 text-cyan-400 bg-cyan-500/10'
                      }`}>
                        {record.path === 'university' ? 'UNIVERSITY' : 
                         (record.path === 'high_school' ? 'HIGH SCHOOL' : 
                         (record.path === 'teacher' ? 'TEACHER' : 'MIDDLE SCHOOL'))}
                      </span>
