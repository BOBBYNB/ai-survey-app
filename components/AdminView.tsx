...
const AdminView: React.FC<AdminViewProps> = ({ onBack }) => {
  ...
  const handleClearDatabase = async () => {
    if (window.confirm('WARNING: This will permanently delete ALL survey records from the CLOUD DATABASE. This action cannot be undone. Continue?')) {
      setLoading(true);
      try {
        const { error } = await supabase
          .from('survey_responses')
          .delete()
          .gt('id', '00000000-0000-0000-0000-000000000000'); // Delete all rows where id is valid UUID
  ...
  const handleAddMockData = () => {
    ...
    setData(updated);
    alert('Mock data added locally only. It will disappear on refresh.');
  };
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 max-w-6xl mx-auto gap-4">
        <div>
          <h1 className="text-2xl font-bold text-cyan-400 tracking-tight flex items-center gap-2">
            <Database size={24} />
            CLOUD_DATA_VISUALIZATION
          </h1>
          <p className="text-xs text-slate-500 mt-1">CONNECTED_TO: SUPABASE</p>
        </div>
      ...
      {loading && data.length === 0 ? (
         <div className="max-w-6xl mx-auto min-h-[400px] flex items-center justify-center">
            <div className="text-cyan-400 animate-pulse flex items-center gap-2">
              <RefreshCw className="animate-spin" /> FETCHING_DATA...
            </div>
         </div>
      ) : data.length === 0 ? (
        <div className="max-w-6xl mx-auto min-h-[400px] flex flex-col items-center justify-center border border-slate-800 border-dashed rounded-xl bg-slate-900/30">
          ...
          <p className="text-slate-600 text-sm">No survey responses found in Cloud DB.</p>
        </div>
      ) : (
        <>
