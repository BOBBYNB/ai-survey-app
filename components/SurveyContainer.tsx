...
const SurveyContainer: React.FC<SurveyContainerProps> = ({ language, onComplete, onBackToHome }) => {
  ...
  const [path, setPath] = useState<'middle' | 'high' | 'uni' | 'teacher' | null>(null);
  ...
  const getCurrentQuestion = (): Question => {
    ...
    if (path === 'teacher') return content.teacherQuestions[currentQIndex];
  ...
  if (path === 'uni') totalQuestions = content.universityQuestions.length;
  else if (path === 'teacher') totalQuestions = content.teacherQuestions.length;
  else totalQuestions = content.middleSchoolQuestions.length;
  ...
  const handleNext = async () => {
    ...
    // Normal Flow
    let isLast = false;
    if (path === 'uni' && currentQIndex === content.universityQuestions.length - 1) isLast = true;
    else if (path === 'teacher' && currentQIndex === content.teacherQuestions.length - 1) isLast = true;
    else if ((path === 'middle' || path === 'high') && currentQIndex === content.middleSchoolQuestions.length - 1) isLast = true;
  ...
  const saveData = async () => {
    try {
      ...
      if (path === 'teacher') dbPath = 'teacher';
